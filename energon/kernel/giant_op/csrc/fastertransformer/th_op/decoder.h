...
public:
  FusedGPTLayer(
    ...
    Tensor output_bias
    ); // the pg should be global parallel group 
  ...
  // void InitModel(int64_t world_size, int64_t rank, int64_t tensor_para_size, int64_t local_rank);
...
private:
  ...
  int64_t head_num_;
  int64_t head_size_;
  int64_t mem_hidden_dim_;
  // T pg_;
  // const c10::intrusive_ptr<c10d::ProcessGroup> pg_;

  // void InitComm(int64_t world_size, int64_t rank, int64_t tensor_para_size, int64_t local_rank, ncclComm_t& tensor_para_nccl_comm);

  // c10::intrusive_ptr<c10d::ProcessGroup::Work> sendNcclUniqueId(ncclUniqueId* ncclId, int dstRank, const c10::intrusive_ptr<c10d::ProcessGroup>& pg);
  // c10::intrusive_ptr<c10d::ProcessGroup::Work> recvNcclUniqueId(ncclUniqueId* ncclId, int srcRank, const c10::intrusive_ptr<c10d::ProcessGroup>& pg);
  // at::Tensor ncclIdTensor; // for communication
};
...
// #ifdef LEGACY_THS
//   torch::jit::class_<torch_ext::FusedGPTLayer>("FusedGPTLayer")
// #else
//   torch::jit::class_<torch_ext::FusedGPTLayer>("FasterTransformer", "Decoder")
// #endif
...
static auto fasterTransformerDecoderTHS = 
    ...
    // .def_pickle(
    // [](const c10::intrusive_ptr<torch_ext::FusedGPTLayer>& self) -> std::vector<Tensor> {
    //   return self->get_pickle_info();
    // },
    // [](std::vector<Tensor> state) -> c10::intrusive_ptr<torch_ext::FusedGPTLayer> {
    //   int head_num = state[26][0].item().to<int>();
    //   int head_size = state[26][1].item().to<int>();
    //   int mem_hidden_dim = state[26][2].item().to<int>();
    //   return c10::make_intrusive<torch_ext::FusedGPTLayer>(head_num, head_size, mem_hidden_dim,
